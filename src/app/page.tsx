  ...
  return (
    <div className="font-sans grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20">
      <main className="flex flex-col gap-[32px] row-start-2 items-center sm:items-start w-full max-w-2xl">
        <section className="w-full border rounded p-4">
          ...
          {session ? (
            <div className="flex flex-col gap-3">
              <div className="text-sm">Signed in as {session.user?.email ?? session.user?.name ?? "user"}</div>
              <form
                action={async () => {
                  "use server";
                  await signOut();
              ...
              >
                <button type="submit" className="rounded border px-3 py-2 text-sm">Sign out</button>
              </form>
            </div>
          ) : (
        ...
        {session && (
          <section className="w-full border rounded p-4">
            <h2 className="font-semibold mb-2">Protected API call</h2>
            <ProtectedCall />
          </section>
        )}
